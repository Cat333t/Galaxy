...
let item_info = ["Тарілка з принтом Марса. Доступна в червоному та білому кольорах", "Кружка з принтом Марса. Доступна в червоному та білому кольорах", "Масштабна модель однієї з ракет Galaxy", "Масштабна модель марсохода (ми купуємо їх у NASA)"]
let item_price_object = document.querySelectorAll(".merch-item-price")
let item_info_object = document.querySelectorAll(".merch-item-descr")

for (let i=0; i<item_info_object.length; i+=1) {
    item_price_object[i].innerHTML = item_price[i]
    item_info_object[i].innerHTML = item_info[i]
}

let cart_amount = document.querySelector(".cart-amount")
let add_to_cart_btns = document.querySelectorAll(".add-to-cart-btn")
for (let i = 0; i<add_to_cart_btns.length; i +=1) {
    add_to_cart_btns[i].addEventListener("click", function(){
        cart_amount.innerHTML = +cart_amount.innerHTML + 1
    })
}
...
function change_theme() {
   ...
   if (current_background == 'url(https://learn.logikaschool.com/uploads/2022/11/bg_space.png)') {
       current_background = 'url(https://learn.logikaschool.com/uploads/2022/11/bg_mars.png)'
   } else {
       current_background = 'url(https://learn.logikaschool.com/uploads/2022/11/bg_space.png)'
   }
...
let current_background = 'url(https://learn.logikaschool.com/uploads/2022/11/bg_mars.png)'
